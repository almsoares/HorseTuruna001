...
type
  ...
  TLocalPaisDBService = class(TInterfacedObject, ILocalPaisDBService)
  private
    FDBCon: IDBConexao;
  ...
  public
    ...
    class Function New: ILocalPaisDBService;
...
constructor TLocalPaisDBService.Create;
begin
  FDBCon := {$ifdef fpc}TDBConexaoZeos{$else}TDBConexaoFireDAC{$endif}.New;
end;

function TLocalPaisDBService.Delete(aID: string): string;
var
...
destructor TLocalPaisDBService.Destroy;
begin
//
  inherited;
end;

function TLocalPaisDBService.Insert(aJson: TJsonObject): string;
begin
//
end;

function TLocalPaisDBService.IsExist(aID: string): Boolean;
var
  lQry: {$ifdef fpc}TZQuery{$else}TFDQuery{$endif};
begin
  try
...
function TLocalPaisDBService.Load(aID: string; aPage, aLimit: Integer): string;
var
  lQry: {$ifdef fpc}TZQuery{$else}TFDQuery{$endif};
...
class function TLocalPaisDBService.New: ILocalPaisDBService;
begin
  Result := Self.Create;
end;

function TLocalPaisDBService.Save(aID, aJson: string): string;
var
  lJson: TJsonObject;
begin
...
function TLocalPaisDBService.Update(aJson: TJsonObject): string;
begin
